...
3. добавлена отправка логов в парсер Андрея (при падении или по команде)
4. Получение ip адреса хоста, на котором запущен бот
5. Пин соообщения с текущей печатью
6. Новый тип камеры `raw_stream` — фото берутся из host_snapshot, видео записывается напрямую из потока без перекодирования (ffmpeg `-c:v copy`). Требуется системный `ffmpeg`. Flip/rotate применяются только к фото и таймлапсам; для видео трансформаций используйте тип `ffmpeg`
...
10. Описать тип камеры `raw_stream`
